...
#define CYCLES_PER_ACCELERATION_TICK ((PBUS_TICKS_PER_MICROSECOND*1000000)/ACCELERATION_TICKS_PER_SECOND)
...
void st_wake_up()
{
  ...
  {
   mPORTGClearBits(xAxis.enablePin.pin);
   mPORTAClearBits(yAxis.enablePin.pin);
   mPORTAClearBits(zAxis.enablePin.pin);
  } else {
    mPORTGSetBits(xAxis.enablePin.pin);
    mPORTESetBits(yAxis.enablePin.pin);
    mPORTASetBits(zAxis.enablePin.pin);
  }
  if (sys.state == STATE_CYCLE) {
    ...
    // Initialize step pulse timing from settings. Here to ensure updating after re-writing.
      ...
      step_pulse_time = -(((settings.pulse_microseconds-2)*PBUS_TICKS_PER_MICROSECOND) >> 3);
    #endif
...
static void set_step_events_per_minute(uint32_t steps_per_minute)
{
  ...
  st.cycles_per_step_event = config_step_timer((PBUS_TICKS_PER_MICROSECOND*1000000*60)/steps_per_minute);
}
